    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += prob * self.beliefs[pos]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] = beliefs[pos] * emissionModel[dis]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        ghostsPossible = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        for i in range(self.numParticles):
            j = i % len(ghostsPossible)
            particles.append(tuple(ghostsPossible[j]))
        self.particles = particles
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    trueDistance = util.manhattanDistance(particle_list[i], pacmanPosition)
                    possible *= emissionModels[i][trueDistance]
            beliefs[tuple(particle)] += possible
            ...
            for p in range(0, self.numParticles):
                ...
                for ghost in range(0, self.numGhosts):
                    if noisyDistances[ghost] == None:
                        particle.append(tuple(self.getJailPosition(ghost)))
                    else:
                        particle.append(util.sample(beliefs)[ghost])
