...
def enhancedFeatureExtractorDigit(datum):
    ...
    def count(x,y):
        ...
        if(y !=  DIGIT_DATUM_HEIGHT - 1):
            napp((x, y+1))
        for (px, py) in next:
            if(features[(px,py)] == 0 and marked[px][py] == 0):
                count(px,py)
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    #features['dis'] = util.manhattanDistance(successor.getPacmanPosition(), successor.getGhostPosition(1))
    features['score'] = successor.getScore()
    x,y = successor.getPacmanPosition()
    features['cap'] = len(successor.getCapsules())
    features['lmove'] = len(successor.getLegalActions())
