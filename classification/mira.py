    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, Cgrid):
            ...
            for iteration in range(self.max_iterations):
                ...
                for i in range(len(trainingData)):
                    ...
                    for l in self.legalLabels:
                        score[l] = cWeights[c][l] * tdata
                    label = score.argMax()
                    if label != trainingLabels[i]:
                        t = min(c, ((cWeights[c][label]-cWeights[c][trainingLabels[i]]) * tdata + 1.0) / (2 * tdata.__mul__(tdata)))
                        temp = util.Counter()
                        for f in tdata:
                            temp[f] = tdata[f] * t
                        cWeights[c][label] -= temp
        ...
        for c in Cgrid:
            ...
            for vdata in validationData:
                cand = util.Counter()
                for l in self.legalLabels:
                    cand[l] = self.weights[l] * vdata
                if cand.argMax() == validationLabels[i]:
                    correct += 1
                i += 1
